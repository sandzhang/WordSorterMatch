...
class WordSorter
{
  public:
    ...
    {
      public:
        Word() : ptr_(NULL), prefix_(0), len_(0) {}
        Word(const char * ptr, const int64_t len)
          : ptr_(ptr), prefix_(*reinterpret_cast<const int64_t *>(ptr_)),
            len_(len)
        {
        }
        ...
        bool operator < (const Word & r) const
        {
          ...
          int64_t l = std::min(len_, r.len_);
          int64_t mlen = std::min(static_cast<int64_t>(sizeof(prefix_)), l);
          int cmp = memcmp(&prefix_, &r.prefix_, mlen);
          //std::cout << " : " << cmp;
          //std::cout << std::endl;
          ...
          {
            if (len_ < r.len_) return true;
            else return false;
          }
          else
          {
            cmp = memcmp(ptr_ + mlen, r.ptr_ + mlen, l - mlen);
            if (cmp < 0) return true;
            else if (cmp > 0) return false;
            else
            {
